defmodule SymphonyElixir.PathSafety do
  ...
  def canonicalize(path) when is_binary(path) do
    ...
    case resolve_segments(root, [], segments, 40) do
      {:ok, canonical_path} ->
        {:ok, canonical_path}
  ...
  defp resolve_segments(root, resolved_segments, [], _remaining), do: {:ok, join_path(root, resolved_segments)}

  defp resolve_segments(_root, _resolved_segments, _segments, 0), do: {:error, :eloop}

  defp resolve_segments(root, resolved_segments, [segment | rest], remaining) do
    ...
    case File.lstat(candidate_path) do
      {:ok, %File.Stat{type: :symlink}} ->
        with {:ok, target} <- :file.read_link_all(String.to_charlist(candidate_path)) do
          ...
          resolve_segments(target_root, [], target_segments ++ rest, remaining - 1)
        end

      {:ok, _stat} ->
        resolve_segments(root, resolved_segments ++ [segment], rest, remaining)
